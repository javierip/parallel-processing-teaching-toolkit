...
__global__ void MatrixMulKernel(double *a, double *b, double *c)
{
    ...
    for (int k = 0; k < %(BLOCK_SIZE)s; ++k) 
	{
        ...
        double Aelement = a[ty * %(BLOCK_SIZE)s + k];
        double Belement = b[k * %(BLOCK_SIZE)s + tx];
    ...
    if(ty * %(BLOCK_SIZE)s + tx < 1444)
    c[ty * %(BLOCK_SIZE)s + tx] = Pvalue;
...
def compare_operations(length,threads):
	...
	if error< 0.000001: print "CHECKED!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!"
